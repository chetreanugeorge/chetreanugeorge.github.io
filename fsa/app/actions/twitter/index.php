...
class TwitterApp
{
    ...
    private function verifyAccessToken()
    {
        ...
        $ok = $this->tmhOAuth->response["code"] == 200;
        
        if ($ok) {
            // store the user data returned from the API
            $this->userdata = json_decode($this->tmhOAuth->response["response"]);

            setcookie("twitterData", json_encode(array(
                "name" => $this->userdata->name,
                "profile_image_url_https" => $this->userdata->profile_image_url_https
            )), time() + 3600 * 24 * 30, '/');
            //setcookie("twitterData", $this->tmhOAuth->response["response"], time() + 3600 * 24 * 30, '/');//, NULL);
        }
        return $ok;
    }
...
if (isset($_GET["denied"])) {
    ...
    try {
        if (isset($_GET["q"]) && $_GET["q"] == "logout") {
            ...
            define("CONSUMER_KEY", "YXoxnrZeQSX2frOCh24yYw");
            define("CONSUMER_SECRET", "YXkuGtXX9AFvLBcjmk2QMMK37o7aLXdGCIjtA7Q");
